...
export default function BookingForm({ onBooked }) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!form.condition) {
      setError("Please select a condition level.");
      return;
    }
    setError("");
    setLoading(true);
    ...
    return (
      <div style={{ textAlign: "center", padding: "32px 0" }}>
        ...
        <div style={{ color: "#64748b", fontSize: 14, marginBottom: 24 }}>
          Queue position: <strong style={{ color: "#0f172a" }}>#{success.queuePosition}</strong>
          &nbsp;·&nbsp; Estimated wait:{" "}
          <strong style={{ color: "#f59e0b" }}>~{success.estimatedWait} min</strong>
        </div>
        <div
          style={{
            background: "#f0fdf4",
        ...
        >
          <div style={{ fontSize: 14, color: "#475569" }}>
            Patient: <strong style={{ color: "#0f172a" }}>{success.name}</strong>
          </div>
          <div style={{ fontSize: 14, color: "#475569" }}>
            Condition:{" "}
            <strong style={{ color: CONDITIONS.find((c) => c.value === success.condition)?.color }}>
              {CONDITIONS.find((c) => c.value === success.condition)?.label}
            </strong>
          </div>
        </div>
        <div>
  ...
  return (
    <form onSubmit={submit}>
      <style>{`input:focus,textarea:focus,select:focus{border-color:#e53935!important;box-shadow:0 0 0 3px rgba(229,57,53,0.15);}`}</style>
      ...
      <label style={{ ...field.label, marginBottom: 12 }}>
        Condition Level *{" "}
        <span style={{ color: "#64748b", fontSize: 11, textTransform: "none", letterSpacing: 0 }}>
            ...
            >
              ...
              <div
                style={{
                  ...
                  height: 18,
                  borderRadius: "50%",
                  border: `2px solid ${selected ? c.color : "#cbd5e1"}`,
              ...
              />
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: selected ? c.color : "#475569" }}>
                  {c.label}
                </div>
